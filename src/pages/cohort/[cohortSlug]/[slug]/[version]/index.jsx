...
`;
  ...
  useEffect(() => {
    ...
    bc.payment({
      ...
      .then(async ({ data }) => {
        const currentPlanFinancing = data?.plan_financings?.find((s) => s?.selected_cohort_set?.slug === cohortSlug);
        const currentSubscription = data?.subscriptions?.find((s) => s?.selected_cohort_set?.slug === cohortSlug);
