...
import { ORIGIN_HOST, isPrismicEnabled } from '../../utils/variables';
import completions from './completion-jobs.json';

const usedPageId = ['home', 'workshops'];
...
function Page({ page }) {
  ...
  return (
    <>
      ...
      <Box className="prismic-body" pt="1rem" px={{ base: '10px', md: '2rem' }} overflow="hidden" pb="5rem" background={backgroundByMode}>
...
export async function getStaticProps({ params, locale, previewData }) {
  ...
  try {
    ...
    if (isDevMode === false && (!isCurrenLang || alreadyUsedPageId)) {
      return { notFound: true };
    }
    ...
    console.error('[uid] Prismic getStaticProps failed:', error?.message || error);
    return { notFound: true };
  }
}
...
export async function getStaticPaths() {
  ...
  try {
    ...
    console.error('[uid] Prismic getStaticPaths failed:', error?.message || error);
    return {
      paths: [],
      fallback: false,
    };
