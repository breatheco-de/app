...
const testPage = () => {
  ...
  const { slugTest, slugTest2 } = router.query;
  console.log('slugTest:::', slugTest);
  console.log('slug:::', slugTest2);
  ...
  useEffect(() => {
    bc.admissions().me().then((res) => {
      ...
      const findCohort = cohorts.find((c) => c.cohort.slug === slugTest);
  ...
  useEffect(() => {
    if (user && user.active_cohort) {
      ...
      bc.syllabus().get(academyId, slugTest2, version).then((res) => {
