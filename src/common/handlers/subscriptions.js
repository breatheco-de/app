  ...
  const process = async () => {
    try {
      ...
      const quarterPlan = quarterly && existsAmountPerQuarter ? {
        ...
        pricePerMonthText: `${currenciesSymbols[data?.currency?.code] || '$'}${(data?.price_per_quarter / 4).toFixed(2)}`,
      ...
      const halfPlan = halfYearly && existsAmountPerHalf ? {
        ...
        pricePerMonthText: `${currenciesSymbols[data?.currency?.code] || '$'}${(data?.price_per_half / 6).toFixed(2)}`,
      ...
      const yearPlan = yearly && existsAmountPerYear ? {
        ...
        pricePerMonthText: `${currenciesSymbols[data?.currency?.code] || '$'}${(data?.price_per_year / 12).toFixed(2)}`,
