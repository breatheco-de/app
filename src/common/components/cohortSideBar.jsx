...
const CohortSideBar = ({ title, cohortCity, professor, assistant, classmates, background }) => {
    ...
    <Box maxW="sm" borderWidth="1px" borderRadius="lg" overflow="hidden" bg={colorMode === 'light' ? background || "lightBlue" : 'featuredDark'}>
      <Box padding={26} >
        ...
        <Box d="flex" alignItems="center">
          <Box>
            <Box width="9px" height="9px" borderRadius="full" bg={professor.active ? "success" : "danger"} position="relative" left="30px" top="11px" border="1px solid white"></Box>
      ...
      <Box padding={26}>
        {assistant ? <><Box>
          ...
          <Grid templateColumns="repeat(5, 1fr)" gap={0}>
            {assistant.map((a, i) => {
              return <Box margin={"6px"}>
                <Box width="9px" height="9px" borderRadius="full" bg={a.active ? "success" : "danger"} position="relative" left="30px" top="11px" border="1px solid white"></Box>
        ...
        <Grid templateColumns="repeat(5, 1fr)" gap={0}>
          {classmates.map((c, i) => {
            return <Box margin={"6px"}>
              <Box width="9px" height="9px" borderRadius="full" bg={c.active ? "success" : "danger"} position="relative" left="30px" top="11px" border="1px solid white"></Box>
              <Image key={i} margin="auto" borderRadius="full" width="39px" height="39px" src="https://storage.googleapis.com/media-breathecode/639857ed0ceb0a5e5e0429e16f7e3a84365270a0977fb94727cc3b6450d1ea9a" />
            </Box>
          })}
        </Grid>
